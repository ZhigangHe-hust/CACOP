...
# import models.CACViT_FPN as CntVit
import models.CACViT_FPN_Selective as CntVit
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for data_iter_step, (samples, gt_density, boxes, m_flag, scales) in enumerate(metric_logger.log_every(data_loader_train, print_freq, header)):
            ...
            # gt_density = torch.clamp(gt_density, min=1e-6)
            
            loss = (output - gt_density) ** 2
            loss = (loss * masks / (384*384)).sum() / (output.shape[0] + 1e-6)
            if not torch.isfinite(loss).all():
                loss = torch.ones_like(loss)  # 替换为有效值
            ...
            for i in range(output.shape[0]): # output.shape[0]==16，因为一个批次有16张图片
                output_i = torch.clamp(output[i], min=1e-6)
                gt_density_i = torch.clamp(gt_density[i], min=1e-6)
                pred_cnt = torch.sum(output_i/60).item()  # 预测的目标数量
                pred_cnt_list.append(pred_cnt)
                output_list.append(output[i])
                gt_cnt = torch.sum(gt_density_i/60).item() # 实际目标数量
        ...
        if args.output_dir and epoch >= 10 and epoch % 1 == 0: # 训练超过10个epoch后，每1个epoch进行一次验证
            # 在验证集上评估当前模型
