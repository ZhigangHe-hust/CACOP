...
def demo(args):
    ...
    model.load_state_dict(
        torch.load('./MODEL_folder/GeCo.pth', weights_only=True)['model'], strict=False,
    )
    ...
    print(f"   Device: {bboxes.device}")
    print(f"   Dtype: {bboxes.dtype}")
    print(f"   First few boxes: {bboxes[:3] if len(bboxes) > 0 else 'No boxes'}")
    ...
    print("\n4. Before Model Input:")
    print(f"   Image shape: {img.shape}")
    print(f"   Boxes shape: {bboxes.shape}")
    print(f"   Image device: {img.device}")
    print(f"   Boxes device: {bboxes.device}")
