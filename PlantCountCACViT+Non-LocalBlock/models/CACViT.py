...
class NonLocalBlock(nn.Module):
    def __init__(self, in_channels, inter_channels=None):
        ...
        self.inter_channels = inter_channels or in_channels // 2  # 512
        if self.inter_channels == 0:    
            self.inter_channels = 1

        self.g = nn.Conv1d(in_channels, self.inter_channels, kernel_size=1) # 1024 -> 512
        self.theta = nn.Conv1d(in_channels, self.inter_channels, kernel_size=1) # 1024 -> 512
        self.phi = nn.Conv1d(in_channels, self.inter_channels, kernel_size=1) # 1024 -> 512
        self.W = nn.Conv1d(self.inter_channels, in_channels, kernel_size=1) # 512 -> 1024
        nn.init.constant_(self.W.weight, 0) # Initialize weights to zero
        nn.init.constant_(self.W.bias, 0)   # Initialize bias to zero

    def forward(self, x):
        """
        x: (B, N, C) 16x576x1024
        """
        x = x.permute(0, 2, 1)  # -> (B, C, N) 16x1024x576
        g_x = self.g(x) # (B, C', N) 16x 512 x 576
        theta_x = self.theta(x).permute(0, 2, 1)  # (B, N, C')
        ...
        W_y = self.W(y) # (B, C, N) 16x1024x576
        z = W_y + x # (B, C, N) 16x1024x576
        return z.permute(0, 2, 1) #  (B, N, C) 16x576x1024
...
class SupervisedMAE(nn.Module):
    ...
    def __init__(self, img_size=384, patch_size=16, in_chans=3, 
        ...
        for i in range(depth):
            ...
            # 在中部插入 Non-local 模块增强上下文建模
            if i == depth // 2:
                self.blocks.append(NonLocalBlock(embed_dim))
    ...
    def forward_encoder(self, x, y, scales=None):
        ...
        for i, blk in enumerate(self.blocks):
            if isinstance(blk, NonLocalBlock):
                x_y = blk(x_y)
                attns.append(None)  # Non-local 无 attn 输出
            else:
                x_y, attn = blk(x_y)
                attns.append(attn)
