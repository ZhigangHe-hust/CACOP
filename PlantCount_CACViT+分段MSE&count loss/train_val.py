...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for data_iter_step, (samples, gt_density, boxes, m_flag, scales) in enumerate(metric_logger.log_every(data_loader_train, print_freq, header)):
            ...
            # mse_loss = (output - gt_density) ** 2
            # mse_loss = (mse_loss * masks / (384*384)).sum() / output.shape[0]

            # # 新增log-count loss
            # pred_count = torch.sum(output, dim=[1,2])  # [B]
            # gt_count = torch.sum(gt_density, dim=[1,2])  # [B]
            # log_count_loss = ((torch.log1p(pred_count) - torch.log1p(gt_count)) ** 2).mean()

            #  # 融合损失
            # loss = mse_loss + 0.1 * log_count_loss
            # loss_value = loss.item()

            # Piecewise MSE loss（已加 mask 和前景加权）
            abs_error = (output - gt_density).abs()
            squared_error = (output - gt_density) ** 2
            tau = 2e-4
            lambda_high = 2.5

            piecewise_loss = torch.where(
                abs_error <= tau,
                squared_error,
                lambda_high * squared_error
            )

            w_fg = 1.5
            w_bg = 1.0
            weighted_loss = (
                w_fg * piecewise_loss * foreground_mask +
                w_bg * piecewise_loss * background_mask
            )

            # 加 mask
            loss_pixel = (weighted_loss * masks / (384 * 384)).sum() / output.shape[0]

            # Count loss
            pred_cnt = output.sum(dim=[1, 2])
            gt_cnt = gt_density.sum(dim=[1, 2])
            loss_count = (pred_cnt - gt_cnt).abs().mean()  # 或平方误差

            # 融合
            alpha = 0.01
            loss = loss_pixel + alpha * loss_count
        ...
        if args.output_dir and train_mae/(len(data_loader_train) * args.batch_size) < min_MAE:# 检查当前epoch的训练MAE是否是迄今为止最好的
            ...
            misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=777) # 保存当前最佳模型：checkpoint-666.pth
        ...
        if args.output_dir and epoch >= 20 and epoch % 1 == 0: # 训练超过30个epoch后，每2个epoch进行一次验证
            ...
            if mae_test < mae:
                ...
                misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=777)
