...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for data_iter_step, (samples, gt_density, boxes, m_flag, scales) in enumerate(metric_logger.log_every(data_loader_train, print_freq, header)):
            ...
            mask = np.random.binomial(n=1, p=0.85, size=[384, 384])
            masks = np.tile(mask, (output.shape[0], 1)).reshape(output.shape[0], 384, 384)
            masks = torch.from_numpy(masks).to(device)
            # 判断前景位置：gt_density > epsilon
            epsilon = 1e-7 
            foreground_mask = (gt_density > epsilon).float() 
...
#             tau	1e-4 ~ 2e-4	不要太小，否则无误差进入高误差段
# lambda_high	2.0 ~ 3.0	强度温和，不至于放大噪声误差
# w_fg	1.5 ~ 2.0	不建议太大，避免训练偏向少数区域
# mask p	≥0.9 或去除	或尝试设为常量 mask，例如中心区域固定保留
