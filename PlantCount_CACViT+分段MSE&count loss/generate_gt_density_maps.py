...
class FSC147Dataset(Dataset):
    ...
    def __init__(
        ...
        with open(
            ...
            os.path.join(self.data_path, 'Train_Test_Val_ours.json'), 'rb'
        ) as file:
            splits = json.load(file)
            self.image_names = splits[split]
        with open(
            # os.path.join(self.data_path, 'annotation_FSC147_384.json'), 'rb'
            os.path.join(self.data_path, 'annotation_ours.json'), 'rb'
        ) as file:
    ...
    def __getitem__(self, idx: int):
        ...
        density_map = torch.from_numpy(np.load(os.path.join(
            self.data_path,
            # 'gt_density_map_adaptive_object_VarV2',
            'gt_density_map_ours',
...
def generate_density_maps(data_path):
    
    density_map_path = os.path.join(
        data_path,
        # f'gt_density_map_adaptive_object_VarV2'
        f'gt_density_map_ours'
    )
    ...
    with open(
        ...
        os.path.join(data_path, 'annotation_ours.json'), 'rb'
    ) as file:
    ...
    for i, (image_name, ann) in enumerate(tqdm(annotations.items())):
        # 加载图像并获取其原始大小
        # image = Image.open(os.path.join(data_path, 'images_384_VarV2', image_name))
        image = Image.open(os.path.join(data_path, 'images_ours', image_name))
        w, h = image.size

        # 处理点位
        points = (torch.tensor(ann['points'], device=device)).float()
        ...
        # 根据图像大小生成密度图
        dmap = torch.zeros(h, w, device=device)
        for p in range(points.size(0)):
            dmap[points[p, 1].long(), points[p, 0].long()] += 1
...
if __name__ == '__main__':
    ...
    generate_density_maps(args.data_path)
