...
def batch_padding(tensor_list, target_dict):
    if tensor_list[0].ndim == 3:
        ...
        for idx, package in enumerate(zip(tensor_list, tensor, density_map, pt_map)):
            ...
            # 获取当前密度图和目标区域的尺寸
            src_density = target_dict[idx]['density_map']
            target_h, target_w = img.shape[1], img.shape[2]
            src_h, src_w = src_density.shape[1:]
            
            # 确保目标区域和源张量尺寸一致
            if src_h > target_h or src_w > target_w:
                # 如果源密度图大于目标区域，裁剪源密度图
                src_density = src_density[:, :target_h, :target_w]
            elif src_h < target_h or src_w < target_w:
                # 如果源密度图小于目标区域，创建临时张量并填充
                temp_density = torch.zeros(1, target_h, target_w, dtype=dtype, device=device)
                temp_density[:, :src_h, :src_w] = src_density
                src_density = temp_density
            
            # 执行安全的复制操作
            pad_density[:, :target_h, :target_w].copy_(src_density)
            
            # 对pt_map执行相同的安全复制操作
            src_pt_map = target_dict[idx]['pt_map']
            if src_h > target_h or src_w > target_w:
                src_pt_map = src_pt_map[:, :target_h, :target_w]
            elif src_h < target_h or src_w < target_w:
                temp_pt_map = torch.zeros(1, target_h, target_w, dtype=dtype, device=device)
                temp_pt_map[:, :src_h, :src_w] = src_pt_map
                src_pt_map = temp_pt_map
            pad_pt_map[:, :target_h, :target_w].copy_(src_pt_map)
...
class Dataset_ours(Dataset):
    def __init__(self, data_dir, data_list, scaling, box_number=3, scale_number=20, min_size=315, max_size=1584, preload=True, main_transform=None, query_transform=None):
        ...
        annotation_file = os.path.join(self.data_dir, 'annotation_ours.json')
        image_classes_file = os.path.join(self.data_dir, 'ImageClasses_ours.txt')
    ...
    def __getitem__(self, idx):
        ...
        if file_name in self.images:
            ...
            image_path = os.path.join(self.data_dir, 'images_ours/' + file_name)
            density_path = os.path.join(self.data_dir, 'gt_density_map_ours/' + file_name.replace('jpg', 'npy'))
...
def build_dataset(cfg, is_train):
    ...
    dataset = Dataset_ours(data_dir=cfg.DIR.dataset,
                            data_list=data_list,
                            scaling=1.0,
...
if __name__ == '__main__':
    ...
    dataset = Dataset_ours(data_dir='D:/dataset/FSC147/',
