...
from dataset_ours import build_dataset, batch_collate_fn 
from engine_ours import evaluate, train_one_epoch, visualization 
...
def main(args):
    ...
    for epoch in range(cfg.TRAIN.start_epoch, cfg.TRAIN.epochs):
        ...
        mae, rmae, rmse, rrmse, r2, flops = evaluate(model, data_loader_val, device, cfg.DIR.output_dir)
        ...
        if cfg.DIR.output_dir:
            with (output_dir / "log.txt").open("a") as f:
                f.write('Epoch %d: loss %.8f, MAE %.2f, rMAE %.2f, RMSE %.2f, rRMSE %.2f, R2 %.2f, FLOPS %.2e, Best MAE %.2f, Best rMAE %.2f, Best RMSE %.2f, Best rRMSE %.2f, Best R2 %.2f, Best FLOPS %.2e \n' % (
                    epoch + 1, loss, mae, rmae, rmse, rrmse, r2, flops, best_mae, best_rmae, best_rmse, best_rrmse, best_r2, best_flops))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--cfg",
        default="config/bmnet+_ours.yaml",
        metavar="FILE",
